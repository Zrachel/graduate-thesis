...
\begin{enumerate}
	\item TP
	TP为true positive 的缩写， 表示分正确的正样本数。

	\item TN
	TN为true negative的缩写， 表示分正确的负样本数。

	\item FP
	FP为false positive的缩写， 表示错分成正样本（实际是负样本）的数目。

	\item FN
	FN为false negative的缩写， 表示错分成负样本（实际是正样本）的数目。

	\item Reco.
	Reco. 为recognition rate的缩写， 识别率， 表示所有样本中分对的比例。

	\item Recall
	Recall （召回率）表示分对的正样本数占所有分对样本数的比例， 即
	\begin{equation}
		Recall = \frac{TP}{TP+TN}
	\end{equation}

	\item Precision
	Precision(精度) 表示分对的正样本数占所有估计成正样本的样本中的比例， 即
	\begin{equation}
...
采用各网络时， A, B两个人的结果分别如表\ref{tab:p300_cnn_A}， \ref{tab:p300_cnn_B}所示。 其中， CNN1, CNN3, MCNN1, MCNN3 \cite{}的基本模型一样， 且也是采用卷积神经网络对数据进行建模。 不同之处是， $L_1$采用卷积神经网络， 在数据78个时间点的每个时间点共享空间域64个通道的权值； $L_2$采用先降采样后卷积（而非我们方法中的先卷积后降采样）的策略以降低模型复杂度，且采用tanh作为激励函数（而我们的PCNN, PsCNN, PwCNN, PConv方法均选用Relu）。 我们将该模型记为CNN1。
...
\centering
  ...
  \hline\hline
   CNN1 & 2021 & 10645 & 4355 & 979 & 70.37 &  0.674 & 0.407 & 0.509\\
  \hline
...
\hline 
  ...
  \hline
  \end{tabular}
  \caption{实验人A —— P300 检测各卷积神经网络结果（结果没更新上去）}
...
将PConv, PwCNN, PsCNN的测试loss随迭代次数变化的曲线画在图\ref{fig:test_loss}。 如图可见， 在测试集上的loss总体上呈下降趋势， 但可能存在局部波动。 最后， 我们将三种方法的测试准确率展现在图\ref{test_acc}中。 将准确率定义为Reco.，可见， 三种方法的测试准确率都较为稳定， PConv的准确率最佳。


\begin{figure*}[htb]
  \centering
  % Requires \usepackage{graphicx}
  \includegraphics[scale=0.8]{Pictures/CNN/test_loss.png}
  \caption{ConvP300Net 训练loss-迭代次数}\label{fig:test_loss}
\end{figure*}

\begin{figure*}[htb]
  \centering
  % Requires \usepackage{graphicx}
  \includegraphics[scale=0.8]{Pictures/CNN/test_accuracy.png}
  \caption{ConvP300Net 训练loss-迭代次数}\label{fig:test_acc}
\end{figure*}
...
\subsection{实现方法}
我们的实现基于Berkeley 大学Jia Yangqing实现的Caffe\cite{}， 为开源代码。 它可以实现在CPU和GPU上运行， 这里我们选用的GPU为NVIDIA出的GT Force 580。 基于cuda进行并行加速。 
